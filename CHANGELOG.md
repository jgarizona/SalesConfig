...
- **HubSpot connector** — full integration plan defined 2026-08-25 (see `HANDOFF.md` §9 for the durable record, plus a visual reference at `https://claude.ai/code/artifact/574c8387-7674-4b36-bbf7-61c91a798e41`): Private App auth (not OAuth, 11 scopes, must be created by a HubSpot Super Admin rather than a visibility-restricted seat — see §9 for why), a new `hubspot_client.py` module (not started), and five concrete interactions (customer/company lookup, Deal lookup via the Associations API, line-item+amount push, pricing-export attach, final-quote attach). Confirmed against the real JLT portal: the native Quote object is blocked (no Revenue Hub license / permission not granted), so the plan targets Deal + Line Item + Notes + Files instead. **Blocking next action:** a HubSpot Super Admin must create the Private App per §9's checklist before any of this can be coded. Sales page still uses a manually-typed Opportunity ID and a stub "Upload Hspt" button in the meantime.
